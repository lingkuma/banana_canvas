...
const ElementPreview: React.FC<ElementPreviewProps> = ({ element, isSelected, onSelect }) => {
    ...
    const renderContent = () => {
        switch (element.type) {
            ...
            default:
                return <div className="text-gray-400 text-sm">{t('unknownElement')}</div>;
        }
    };
...
export const TrashModal: React.FC<TrashModalProps> = ({ elements, onClose, onRestore, onDelete }) => {
    ...
    const handleEmptyTrash = () => {
        if (window.confirm(t('confirmEmptyTrash', { count: elements.length }))) {
        ...
        <div className="absolute inset-0 z-40 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-lg flex-shrink-0">
                    <h2 className="text-xl font-bold text-gray-800">{t('trashTitle', { count: elements.length })}</h2>
                ...
                <div className="flex-grow p-4 bg-gray-100 overflow-y-auto">
                    {sortedElements.length > 0 ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-gray-500">
                            ...
                            <p className="text-lg font-semibold">{t('trashEmpty')}</p>
                        </div>
                    )}
                </div>
                ...
                <div className="p-4 border-t flex justify-between items-center bg-gray-50 rounded-b-lg flex-shrink-0">
                    <div className="text-sm text-gray-600">
                       {selectedIds.length > 0 ? t('selectedItems', { count: selectedIds.length }) : t('selectTrashItems')}
                    </div>
                    <div className="flex gap-2">
                        <button onClick={handleRestore} disabled={selectedIds.length === 0} className="px-4 py-2 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed">{t('restore')}</button>
                        <button onClick={handleDelete} disabled={selectedIds.length === 0} className="px-4 py-2 text-sm bg-red-600 text-white rounded-md hover:bg-red-700 disabled:bg-gray-300 disabled:cursor-not-allowed">{t('deletePermanently')}</button>
                        <button onClick={handleEmptyTrash} disabled={elements.length === 0} className="px-4 py-2 text-sm bg-gray-700 text-white rounded-md hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed">{t('emptyTrash')}</button>
                    </div>
                </div>
            </div>
        </div>
