    ...
    return (
        <>
            <button
                ...
                aria-label={isOpen ? t('closeHistory') : t('openHistory')}
            >
            ...
            >
                <div>
                    <h1 className="text-xl font-bold text-gray-800">{t('generationHistory')}</h1>
                    <p className="text-sm text-gray-600 mt-1">{t('historyHelp')}</p>
                </div>

                <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4">
                    {annotationPreview && (
                        <div className="border rounded-lg p-2 bg-gray-100/50 shadow-sm">
                            <div className="text-xs font-semibold text-gray-600 mb-2">{t('latestAnnotation')}</div>
                            <img src={annotationPreview} alt={t('latestAnnotation')} className="w-full h-auto object-contain rounded-md" />
                        </div>
                    )}
                        ...
                        <div key={item.id} className="border rounded-lg p-2 flex flex-col gap-2 bg-gray-100/50 shadow-sm">
                            <div className="flex items-center justify-between gap-2">
                                <div className="min-w-0">
                                    <div className="text-sm font-semibold text-gray-800 truncate">
                                        {item.status === 'generating' ? t('generating') : item.status === 'failed' ? t('failed') : t('completed')}
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        {item.images.length > 0 ? t('imagesCount', { count: item.images.length }) : t('requestsCount', { count: item.requestedCount })}
                                    </div>
                                </div>
                            ...
                            {item.status === 'generating' && (
                                <div className="flex flex-col items-center justify-center text-gray-700 p-4 border border-dashed rounded-lg bg-white/60">
                                    ...
                                    <p className="text-sm font-semibold">{t('generatingEllipsis')}</p>
                                    <p className="text-xs text-gray-500">{t('parallelTask')}</p>
                                </div>
                            )}
